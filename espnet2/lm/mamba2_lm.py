...
from espnet2.asr.encoder.mamba2_encoder import Mamba2EncoderLayer, SerialBiMamba2EncoderLayer
...
def compute_flip_fn_and_mask_parallel(sp_token_idx, input_seq, d_xz, d_aux):
    mask = (input_seq != 0)[:, None, :]
    bs, seq_len = input_seq.shape
    flip_idxs = [
        torch.arange(bs * (d_xz + d_aux) * seq_len, device=input_seq.device),
        torch.arange(bs * d_xz // 2 * seq_len, device=input_seq.device),
    ]

    flip_idx_before_ssm = flip_idxs[0].view(bs, (d_xz + d_aux), seq_len)
        ...
        **factory_kwargs,
        ...
        if prefix_bidir:
            if prefix_bidir_mode == "serial":
                ...
                encoder_layer_cls = SerialBiMamba2EncoderLayer
            elif prefix_bidir_mode == "parallel":
                ssm_cfg["bidirectional"] = True
                encoder_layer_cls = Mamba2EncoderLayer
        else:
            encoder_layer_cls = Mamba2EncoderLayer
        ...
        if init_rescale:
            logging.info("Rescaling weights in Mamba2Encoder")
        ...
        n_heads = d_inner // ssm_cfg["headdim"]
        self.d_xz = 2 * d_inner
        self.d_aux = 2 * ssm_cfg["d_state"] + n_heads
